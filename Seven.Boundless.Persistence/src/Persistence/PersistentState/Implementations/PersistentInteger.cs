...
public sealed class PersistentInteger(int min, int max, int defaultValue) : PersistentState {
	...
	/// <summary>
	/// Represents an integer value that can be persisted.
	/// </summary>
	/// <param name="Val">The underlying integer value.</param>
	[Serializable]
	public readonly record struct Value(int Val) : IPersistentValue {
		/// <inheritdoc/>
		public static implicit operator int(Value key) => key.Val;
		/// <inheritdoc/>
		public static implicit operator Value(int key) => new(key);
	}
}